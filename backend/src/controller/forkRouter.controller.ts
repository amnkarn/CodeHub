...
export const forkRepository = asyncHandler(async (req: Request, res: Response) => {
    ...
    const targetRepo = await findRepoWithAccess(ownerName, repoName, userId);
...
export const getForkedRepositories = asyncHandler(async (req: Request, res: Response) => {
    const parsedParams = forkParamsSchema.safeParse(req.params);
    if(!parsedParams.success) {
        return res.status(400).json({
            message: "Invalid parameters"
